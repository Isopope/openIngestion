...
class MinerUChef(BaseChef):
    ...
    * :class:`~openingestion.document.FetchedDocument` → unwrapped and
    ...
    def __init__(
        ...
        #: Set by :meth:`process` after each call; the directory that contains
        #: ``*_content_list.json`` and the ``images/`` folder for the last
        #: processed source.  Used by :func:`~openingestion.ingest` to pass
        #: the correct base directory to :class:`~openingestion.refinery.RagRefinery`.
    ...
    def process(
        ...
        if target.is_file() and target.suffix == ".json":
            ...
            self.last_output_dir = target.parent
    ...
    def map_to_blocks(self, raw_items: list[dict]) -> list[ContentBlock]:
        ...
        for idx, item in enumerate(raw_items):
            ...
            block_index = idx  # globally unique across all pages

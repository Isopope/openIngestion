...
def _build_pattern(delim: list[str]) -> re.Pattern:
    """Compile a regex pattern from the delimiter list.

    Delimiters are sorted longest-first to avoid prefix shadowing
    (e.g. ``"..."`` before ``"."``).
    """
    sorted_delim = sorted(delim, key=len, reverse=True)
    pattern = "|".join(re.escape(d) for d in sorted_delim)
...
def _split_text(
    ...
    Args:
        text:                       Input text.
        pattern:                    Compiled regex with a single capturing group
    ...
    """
    # re.split with a capturing group produces alternating [text, delim, text, delim, ...]
    ...
    while i < len(parts):
        ...
        i += 1
        if i < len(parts):
            delim_found = parts[i]  # captured delimiter
            i += 1
            if include_delim == "prev":
                chunk = chunk + delim_found
            elif include_delim == "next":
                # Prepend delimiter to the start of the next text fragment
                if i < len(parts):
                    parts[i] = delim_found + parts[i]
                # delimiter at the very end of text -> discard
            # include_delim is None -> delimiter discarded
...
class SentenceChunker(BaseChunker):
    ...
    def __init__(
        ...
        # Pre-compile the split pattern once
        self._pattern = _build_pattern(self.delim)

...
class BaseEmbedder(ABC):
    ...
    def similarity(a: "list[float]", b: "list[float]") -> float:
        ...
        Raises:
            ImportError: If numpy is not installed (``pip install openingestion[semantic]``).
        """
        if not _NUMPY_AVAILABLE:
            raise ImportError(
                "numpy is required for similarity computation. "
                "Install it with: pip install openingestion[semantic]"
            )
